...
public class Percolation {
    ...
    public void open(int i, int j){
        if (i == 0){
            quickUnionUF.union(system[i][j],numberOfCells*numberOfCells);

        }
        if (i == numberOfCells-1 && quickUnionUF.connected(system[i][j],numberOfCells*numberOfCells)){
            quickUnionUF.union(system[i][j],numberOfCells*numberOfCells+1);
        }
...
//    return quickUnion.connected(numberOfCells*numberOfCells,numberOfCells*numberOfCells+1);
    ...
    public boolean isFull(int row, int col) {
        return quickUnionUF.connected(system[row][col], numberOfCells*numberOfCells);
    }
    // чи протікає система?
}
